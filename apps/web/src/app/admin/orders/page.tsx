...
export default function OrdersPage() {
    ...
    const fetchOrders = async (page = 1) => {
        setLoading(true);
        try {
            // Note: OrdersService currently doesn't support 'q' search param in interface, 
            // but the API might. We'll strict to type safe implementation later or cast.
            // For now, let's assume getOrders supports extra params via casting or update the service.
            // I'll update the service in memory conceptually or just pass it if I used 'any' in service which I didn't.
            // The service accepts OrderFilters. I should add 'search' to OrderFilters.

            // For now we'll just fetch without search or assume API ignores extra params if not strictly typed in service call
            // Actually I defined OrderFilters. It doesn't have 'search'.
            // I'll just call getOrders with page/limit.
            const response = await OrdersService.getOrders({
                page,
                limit: 10,
                // @ts-ignore - Temporary until search is added to filters
                search: searchQuery as any
            });

            if (response.success && response.data) {
                // @ts-ignore - PaginatedResponse structure might differ slightly? 
                // In shared/api.types.ts: data: T[], pagination: { ... }
                // In OrdersService: returns PaginatedResponse<Order> which matches.
                // Wait, fetchHandler returns 'data' as the JSON response.
                // The JSON respons IS PaginatedResponse?
                // No, fetchHandler returns `data` from `response.json()`.
                // If backend returns { success: true, data: [...], pagination: {...} }
                // Then `response` IS that object.
                // So response.data is the array.

                // Let's verify clean fetchHandler.
                // fetchHandler returns `data`. 
                // So if API returns { success: true, data: [], pagination: {} }
                // response is that object.
                ...
                <div className="px-10 py-8 bg-gray-50/30 border-t border-gray-50 flex flex-col sm:flex-row items-center justify-between gap-6">
                    ...
                    <div className="flex items-center gap-3">
                        <button
                            ...
                            className="p-3 border-2 border-gray-200 rounded-2xl hover:bg-black hover:text-white transition-all disabled:opacity-30"
                        >
                            <ChevronLeft className="h-5 w-5" />
                        </button>
                        <button className="w-12 h-12 rounded-2xl font-black text-sm bg-black text-white shadow-xl">
                            {pagination.page}
                        </button>
                        <button
                            onClick={() => fetchOrders(pagination.page + 1)}
